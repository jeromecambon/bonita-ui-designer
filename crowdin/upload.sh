...
usage() {
  ...
  echo "  Generate pot files containing translations keys for UI Designer community and subscription"
...
upload_translations() {
  ...
  curl -F "files[ui-designer/lang-template.pot]=@community/backend/webapp/src/main/resources/i18n/lang-template-$2.po" \
       -F "language=$1" \
  ...
  curl -F "files[ui-designer/lang-template-sp.pot]=@subscription/backend/src/main/resources/i18n/lang-template-$2.po" \
       -F "language=$1" \
       -F "auto_approve_imported=1" \
       -F "import_duplicates=1" \
       -F "import_eq_suggestions=1" \
...
npm_pot community/frontend/
npm_pot community/backend/webapp/
npm_pot subscription/frontend/
npm_pot subscription/backend/

echo "Concatenating pot files..."
cat_pot $BASE_DIR/community $BUILD_DIR/lang_template.pot
cat_pot $BASE_DIR/subscription $BUILD_DIR/lang_template-sp.pot

add_crowdin_directory "$BRANCH_NAME/ui-designer"
echo "Uploading community pot to $PROJECT crowdin project ..."
upload_sources \
...
echo "Uploading sp pot to $PROJECT crowdin project ..."
upload_sources \
    "files[ui-designer/lang-template-sp.pot]=@$BUILD_DIR/lang_template-sp.pot"  \
    "branch=$BRANCH_NAME" \
    "export_patterns[ui-designer/lang-template-sp.pot]=/ui-designer/sp/lang-template-%locale%.po"
